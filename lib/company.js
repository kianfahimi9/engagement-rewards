...
export async function ensureCommunityExists(companyContext) {
  try {
    ...
    // Try to detect forum and chat experiences by actually calling the SDK
    let forumExperienceId = null;
    let chatExperienceId = null;
    
    for (const exp of companyContext.allExperiences) {
      // Try to list forum posts - if successful, it's a forum
      if (!forumExperienceId) {
        try {
          ...
          console.log('✅ Detected forum experience:', exp.id, exp.name);
        } catch (err) {
          // Not a forum, continue
        }
      }
      
      // Try to list chat messages - if successful, it's a chat
      if (!chatExperienceId) {
        try {
          ...
          console.log('✅ Detected chat experience:', exp.id, exp.name);
        } catch (err) {
          // Not a chat, continue
        }
      }
      ...
      if (forumExperienceId && chatExperienceId) break;
    }
    ...
    console.log('   Forum Experience:', forumExperienceId || 'Not found');
    console.log('   Chat Experience:', chatExperienceId || 'Not found');
