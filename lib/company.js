  ...
  async (experienceId) => {
    try {
      ...
      if (envCompanyId) {
        ...
        // Try to cache it in DB for future use (ignore if already exists)
        try {
          await supabase
            .from('communities')
            .upsert({
          ...
          // Ignore DB cache errors - not critical
          console.log('ℹ️  DB cache update failed (non-critical):', dbError.message);
        }
      ...
      throw new Error(
        ...
        `Please add NEXT_PUBLIC_WHOP_COMPANY_ID to your environment variables or ` +
