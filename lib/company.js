...
export const getCompanyFromExperience = cache(
  async (experienceId) => {
    try {
      ...
      // NEW SDK: We need to get the company_id first, then fetch company details
      // Since there's no direct getExperience method, we'll need to find it via listing
      // First, let's try to get it from the database if it exists
      ...
      if (existingExp?.whop_company_id) {
        console.log('✅ Found company from DB cache:', existingExp.whop_company_id);
        // Fetch fresh company data
        const company = await whopSdk.companies.retrieve(existingExp.whop_company_id);
        
        return {
          companyId: company.id,
          companyTitle: company.title,
          companyRoute: company.route,
          companyLogo: null, // Companies API doesn't return logo in new SDK
          experience: {
            id: experienceId,
            name: null, // We'll get this from listing experiences
            description: null
          }
        };
      }
      
      // If not in DB, we need to scan - this is expensive but only happens once
      // We'll need the company_id from headers or context
      throw new Error('Company ID not found in database. Please ensure the app is properly installed.');
