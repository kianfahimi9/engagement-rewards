...
export const verifyUser = cache(
  async (experienceId, level) => {
    try {
      ...
      // Check if user is the company owner (not just admin/moderator)
      let isOwner = false;
      try {
        const authorizedUsersResult = await whopSdk.companies.listAuthorizedUsers({
          companyId: companyContext.company.companyId
        });
        
        const authorizedUser = authorizedUsersResult.authorizedUsers.find(
          user => user.userId === userId
        );
        
        isOwner = authorizedUser?.role === 'owner';
        console.log(`👤 User ${userId} owner status:`, isOwner, `(role: ${authorizedUser?.role})`);
      } catch (error) {
        console.error('Failed to check owner status:', error.message);
      }
