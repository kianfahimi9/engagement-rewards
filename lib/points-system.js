...
export function calculateForumPostPoints(post, allPosts = []) {
  ...
  // Debug logging for posts that should have replies
  if (post.id === 'post_1CTaduCmCgFSVYJBQQ2wwB') {
    console.log('🔍 DEBUG - Reply calculation for parent post:', {
      postId: post.id,
      totalPostsToCheck: allPosts.length,
      postsWithThisAsParent: allPosts.filter(p => p.parentId === post.id).length,
      postsWithParentId: allPosts.filter(p => p.parentId).map(p => ({ id: p.id, parentId: p.parentId })),
      qualityRepliesFound: qualityReplies.length,
      qualityReplyIds: qualityReplies.map(r => r.id)
    });
  }

  breakdown.replies = qualityReplies.length * POINT_VALUES.FORUM_REPLY;
