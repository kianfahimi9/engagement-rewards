 ...
 * @param {string} companyId - Whop company ID
 * @returns {Promise<{balance: number, currency: string}>}
 */
export async function getCompanyBalance(companyId) {
  try {
    // Company's ledger account ID is typically the company ID
    const ledgerAccount = await whopSdk.ledgerAccounts.retrieve(companyId);
    
    // Get USD balance (or first available currency)
    const usdBalance = ledgerAccount.balances.find(b => b.currency === 'usd');
    const balance = usdBalance || ledgerAccount.balances[0];
    
    return {
      balance: balance?.balance || 0,
      availableBalance: balance?.balance - balance?.reserve_balance || 0,
      currency: balance?.currency || 'usd',
      pendingBalance: balance?.pending_balance || 0,
      reserveBalance: balance?.reserve_balance || 0
    };
  } catch (error) {
...
export async function distributePrizePool(prizePoolId, companyId) {
  try {
    ...
    const distributionPercentages = [30, 20, 15, 10, 8, 6, 4, 3, 2, 2];
