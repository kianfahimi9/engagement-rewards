...
export async function distributePrizePool(prizePoolId, companyId) {
  try {
    ...
    // regardless of number of winners (1-10)
    const calculateProportionalPercentages = (numWinners) => {
      if (numWinners === 1) return [100];
      if (numWinners === 2) return [69.23, 30.77]; // Maintains ~2.25x ratio
      if (numWinners === 3) return [54.55, 25.00, 20.45]; // Maintains relative ratios
      
      // For 4+ winners, use the original fixed percentages as weights
    ...
    const distributionPercentages = calculateProportionalPercentages(winners.length);
    
    console.log(`📊 Distributing to ${winners.length} winners with proportional percentages:`, 
      distributionPercentages.map((p, i) => `Rank ${i+1}: ${p.toFixed(2)}%`).join(', '));
    ...
    await supabase
      ...
      .eq('whop_checkout_id', prizePoolId);
