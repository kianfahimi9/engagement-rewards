...
export async function syncForumPosts(communityId, forumExperienceId) {
  ...
  try {
    // Fetch forum posts from Whop - try different parameter combinations
    // Based on research, forums might not support includeReplies like chat does
    const response = await whopSdk.forums.listForumPostsFromForum({
      experienceId: forumExperienceId,
      limit: 100, // Increase limit to ensure we get all posts including replies
    });
    ...
    // Debug: Log FULL first post structure to see ALL available fields
    if (posts.length > 0) {
      console.log('🔍 DEBUG - FULL first post structure:', JSON.stringify(posts[0], null, 2));
      ...
      console.log(`📊 Post breakdown: ${topLevelPosts.length} top-level, ${replyPosts.length} replies`);
      
      // Check if posts have a replies array or similar field
      console.log('🔍 Available fields on first post:', Object.keys(posts[0]));
    }
