...
export async function syncForumPosts(communityId, forumExperienceId) {
  ...
  try {
    // Use the NEW forumPosts API (not the old forums.listForumPostsFromForum)
    // This should include view_count, like_count, and comment_count
    const response = await whopSdk.forumPosts.list({
      forum_id: forumExperienceId,
      limit: 100,
    });
    ...
    console.log(`Found ${posts.length} forum posts from NEW API`);
    
    // Debug: Log first post to see if we have comment_count
    if (posts.length > 0) {
      console.log('🔍 DEBUG - First post from NEW API:', JSON.stringify(posts[0], null, 2));
      console.log('🔍 Has comment_count?', 'comment_count' in posts[0]);
      console.log('🔍 Has view_count?', 'view_count' in posts[0]);
    }
