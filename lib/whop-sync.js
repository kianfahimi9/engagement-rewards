  ...
  try {
    ...
    for (const post of posts) {
      ...
      if (existing) {
        ...
        if (existing.points !== points) {
          await supabase
            ...
            .update({
              ...
              is_pinned: isPinned,
              updated_at: new Date(parseInt(post.updated_at) * 1000).toISOString(), // Convert seconds to milliseconds
            })
            .eq('whop_post_id', post.id);
        ...
        // Insert post
        // NOTE: Whop SDK returns created_at/updated_at as Unix timestamps in SECONDS
        const { data: insertedPost, error: insertError } = await supabase
          ...
          .insert({
            ...
            points: points,
            points_breakdown: breakdown,
            created_at: new Date(parseInt(post.created_at) * 1000).toISOString(), // Convert seconds to milliseconds
            updated_at: new Date(parseInt(post.updated_at) * 1000).toISOString(), // Convert seconds to milliseconds
          })
          .select();
