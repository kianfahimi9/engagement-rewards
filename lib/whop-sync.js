...
export async function syncForumPosts(communityId, forumExperienceId) {
  ...
  try {
    ...
    // Debug: Log first post structure to understand reply structure
    if (posts.length > 0) {
      console.log('🔍 DEBUG - First post structure:', JSON.stringify({
        id: posts[0].id,
        parentId: posts[0].parentId,
        viewCount: posts[0].viewCount,
        stats: posts[0].stats,
        hasParentId: !!posts[0].parentId
      }, null, 2));
