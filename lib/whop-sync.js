...
export async function syncForumPosts(communityId, forumExperienceId) {
  ...
  try {
    // STEP 1: Fetch top-level forum posts
    ...
    console.log(`Found ${allPosts.length} top-level forum posts`);
    
    // STEP 2: For each top-level post, try to fetch its replies
    // This is a workaround since the SDK doesn't include replies by default
    const topLevelPosts = allPosts.filter(p => !p.parentId);
    
    for (const parentPost of topLevelPosts) {
      try {
        // Attempt to fetch replies for this specific post
        const repliesResponse = await whopSdk.forums.listForumPostsFromForum({
          experienceId: forumExperienceId,
          parentId: parentPost.id, // Try filtering by parent ID
          limit: 50,
        });
        
        if (repliesResponse?.posts && repliesResponse.posts.length > 0) {
          console.log(`📝 Found ${repliesResponse.posts.length} replies for post ${parentPost.id}`);
          allPosts = [...allPosts, ...repliesResponse.posts];
        }
      } catch (error) {
        // If parentId filtering isn't supported, this will fail silently
        console.log(`⚠️  Could not fetch replies for post ${parentPost.id}: ${error.message}`);
      }
    }
    
    console.log(`📊 Total posts after fetching replies: ${allPosts.length}`);
    
    // Deduplicate posts by ID (we're getting duplicates)
    const uniquePosts = Array.from(new Map(allPosts.map(p => [p.id, p])).values());
    console.log(`✅ Deduplicated: ${allPosts.length} → ${uniquePosts.length} unique posts`);
    allPosts = uniquePosts;
    
    // Debug: Log first post structure to see ALL available fields
    if (allPosts.length > 0) {
      console.log('🔍 DEBUG - FULL first post structure:', JSON.stringify(allPosts[0], null, 2));
      
      // Count posts with parentId (these are replies)
      const replyPosts = allPosts.filter(p => p.parentId);
      const topLevelOnly = allPosts.filter(p => !p.parentId);
      console.log(`📊 Post breakdown: ${topLevelOnly.length} top-level, ${replyPosts.length} replies`);
      
      // Check if posts have a replies array, comments field, or stats field
      console.log('🔍 Available fields on first post:', Object.keys(allPosts[0]));
      
      // Check if there's a stats field with reply count
      if (allPosts[0].stats) {
        console.log('🔍 Stats field found:', JSON.stringify(allPosts[0].stats, null, 2));
      }
      
      // Log a few sample post IDs to check for duplicates
      console.log('🔍 First 15 post IDs:', allPosts.slice(0, 15).map(p => ({ id: p.id, parentId: p.parentId })));
    }
    ...
    for (const post of allPosts) {
      ...
      // Debug: Log post structure to understand reply data
      if (post.id === 'post_1CTaduCmCgFSVYJBQQ2wwB' || post.parentId === 'post_1CTaduCmCgFSVYJBQQ2wwB') {
        console.log('🔍 DEBUG - Post with known reply:', {
          id: post.id,
      ...
      // Calculate points (pass allPosts to include replies in calculation)
      const { points, breakdown } = calculateForumPostPoints(post, allPosts);
