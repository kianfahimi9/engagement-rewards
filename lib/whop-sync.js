...
export async function syncForumPosts(communityId, forumExperienceId) {
  ...
  try {
    ...
    for await (const post of whopSdk.forumPosts.list({ experience_id: forumExperienceId })) {
      // Fetch individual post to get accurate like_count
      // The list endpoint doesn't return real-time like counts
      try {
        const fullPost = await whopSdk.forumPosts.retrieve(post.id);
        
        // Debug: Log raw API response for posts with likes
        if (post.id === 'post_1CUF81GcpsQFoNkhBibA9t') {
          console.log('🔍 RAW API RESPONSE for post_1CUF81GcpsQFoNkhBibA9t:', JSON.stringify(fullPost, null, 2));
        }
