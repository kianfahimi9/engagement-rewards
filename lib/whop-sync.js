    ...
    for (const post of posts) {
      ...
      if (existing) {
        ...
        if (existing.points !== points) {
          ...
          // Update user's leaderboard points when post points change
    ...
    for (const message of messages) {
      ...
      if (existing) {
        ...
        if (existing.points !== points) {
          ...
          // Update user's leaderboard points when message points change
...
async function updateUserPoints(whopUserId, communityId) {
  ...
  const totalPoints = Math.round((posts?.reduce((sum, post) => sum + (Number(post.points) || 0), 0) || 0) * 10) / 10;

  console.log(`💰 Updating points for user ${whopUserId}:`, {
    postsCount: posts?.length || 0,
    individualPoints: posts?.map(p => p.points) || [],
    totalPoints
  });
  ...
  for (const period of periods) {
    // Calculate proper period_start based on period type
    const now = new Date();
    let periodStart;
    ...
    if (period === 'weekly') {
      ...
      // all_time: use a fixed start date
